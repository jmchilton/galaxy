...
class ContainerFinder:
    ...
    def _container_registry_for_destination(self, destination_info):
        ...
        destiantion_container_registry = None
        if destination_id and destination_id not in self.destination_container_registeries:
            if 'container_resolvers' in destination_info:
                destiantion_container_registry = ContainerRegistry(self.app_info, destination_info=destination_info, mulled_resolution_cache=self.mulled_resolution_cache)
                self.destination_container_registeries[destination_id] = destiantion_container_registry
        elif not destination_id and 'container_resolves' in destination_info:
            destiantion_container_registry = ContainerRegistry(self.app_info, destination_info=destination_info, mulled_resolution_cache=self.mulled_resolution_cache)

        if destiantion_container_registry is None and destination_id and destination_id in self.destination_container_registeries:
            destiantion_container_registry = self.destination_container_registeries[destination_id]

        return destiantion_container_registry or self.default_container_registry
...
class ContainerRegistry:
    ...
    def __build_container_resolvers(self, app_info, destination_info):
        ...
        if destination_info is not None:
            conf_file = getattr(destination_info, 'container_resolvers_config_file', app_conf_file)
            conf_dict = getattr(destination_info, 'container_resolvers_config_dict', app_conf_dict)
        else:
