...
class StepDiff(BaseModel):
    ...
    benign_artifact: Optional[BenignArtifact] = None
...
def _classify_missing_value(orig_val: Any) -> tuple[DiffSeverity, Optional[BenignArtifact]]:
...
def _classify_value_mismatch(orig_val: Any, after_val: Any) -> tuple[DiffSeverity, Optional[BenignArtifact]]:
...
class RoundTripValidationResult(BaseModel):
    ...
    workflow_path: str
    format2_dict: Optional[dict] = Field(default=None, exclude=True)
    reimported_dict: Optional[dict] = Field(default=None, exclude=True)
    conversion_result: Optional[RoundTripResult] = None
    diffs: Optional[list[StepDiff]] = None
    step_id_mapping: Optional[StepIdMappingResult] = None
