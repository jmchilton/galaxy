...
class DatabaseOperationTool(Tool):
    ...
    def _add_datasets_to_history(self, history, elements_dict_or_list):
        datasets = []
        if isinstance(elements_dict_or_list, list):
            for element_object in elements_dict_or_list:
                if getattr(element_object, "history_content_type", None) == "dataset":
                    datasets.append(element_object)
        else:
            for element_object in elements_dict_or_list.values():
                if getattr(element_object, "history_content_type", None) == "dataset":
                    datasets.append(element_object)
...
class MergeCollectionTool(DatabaseOperationTool):
    ...
    def produce_outputs(self, trans, out_data, output_collections, incoming, history, **kwds):
        ...
        self._add_datasets_to_history(history, new_elements)
        output_collections.create_collection(
            next(iter(self.outputs.values())), "output", elements=new_elements
        )
...
class FilterFromFileTool(DatabaseOperationTool):
    ...
    def produce_outputs(self, trans, out_data, output_collections, incoming, history, **kwds):
        ...
        self._add_datasets_to_history(history, filtered_elements)
        self._add_datasets_to_history(history, discarded_elements)
        output_collections.create_collection(
