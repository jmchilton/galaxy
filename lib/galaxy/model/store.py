...
class ModelImportStore(object):
    ...
    def perform_import(self, history=None, new_history=False, job=None):
        ...
        hdcas_by_key = {}
        lddas_by_key = {}
        hda_copied_from_sinks = {}
        # TODO: replicate copied from sinks logic with HDCAs and test.
        ...
        for dataset_attrs in datasets_attrs:
            ...
            if 'id' in dataset_attrs:
                ...
                if model_class == "HistoryDatasetAssociation":
                    hdas_by_key[dataset_attrs[object_key]] = dataset_instance
                else:
                    lddas_by_key[dataset_attrs[object_key]] = dataset_instance
        ...
        for dataset_attrs in datasets_attrs:
            ...
            if dataset_attrs[object_key] not in hdas_by_key:
                continue

            hda = hdas_by_key[dataset_attrs[object_key]]
            ...
            for i, copied_from_id in enumerate(copied_from_chain):
                filter_id = (i != len(copied_from_chain) - 1) and (copied_from_id not in hdas_by_key)
                if not filter_id:
                    filtered_copied_from_chain.append(copied_from_id)

            copied_from_chain = filtered_copied_from_chain

            # Re-establish the chain if we can.
            copied_from_id = copied_from_chain[0]
            if copied_from_id in hdas_by_key:
                hda.copied_from_history_dataset_association = hdas_by_key[copied_from_id]
            else:
                ...
                if copied_from_id in hda_copied_from_sinks:
                    hda.copied_from_history_dataset_association = hdas_by_key[hda_copied_from_sinks[copied_from_id]]
                else:
                    hda_copied_from_sinks[copied_from_id] = dataset_attrs[object_key]
        ...
        for library_attrs in libraries_attrs:
            ...
            def import_folder(folder_attrs):
                ...
                self.sa_session.add(library_folder)
                self.sa_session.flush()
                ...
                for ld_attrs in folder_attrs.get("datasets", []):
                    ...
                    if 'ldda' in ld_attrs:
                        ldda = lddas_by_key[ld_attrs["ldda"][object_key]]
            ...
            libraries_by_key[library_attrs[object_key]] = library
        ...
        for collection_attrs in collections_attrs:
            ...
            hdca = hdcas_by_key[collection_attrs[object_key]]
            if "implicit_input_collections" in collection_attrs:
                ...
                for implicit_input_collection in implicit_input_collections:
                    ...
                    if input_collection_identifier in hdcas_by_key:
                        input_dataset_collection = hdcas_by_key[input_collection_identifier]
        ...
        for job_attrs in jobs_attrs:
            ...
            def _find_hda(input_key):
                hda = None
                if input_key in hdas_by_key:
                    hda = hdas_by_key[input_key]
                if input_key in hda_copied_from_sinks:
                    hda = hdas_by_key[hda_copied_from_sinks[input_key]]
                return hda

            def _find_hdca(input_key):
                hdca = None
                if input_key in hdcas_by_key:
                    hdca = hdcas_by_key[input_key]
                # if input_key in hda_copied_from_sinks:
                #    hcda = hda_copied_from_sinks[input_key]
                return hdca
...
class BagArchiveImportModelStore(DirectoryImportModelStoreLatest):
    ...
    def __init__(self, app, user, bag_archive, import_options=None):
        ...
        super(BagArchiveImportModelStore, self).__init__(app, user, archive_dir, import_options=import_options)
    ...
    def export_library(self, library, include_hidden=False, include_deleted=False):
        ...
        def collect_datasets(library_folder):
            for library_dataset in library_folder.datasets:
                ...
                self.included_datasets[ldda.id] = (ldda, True)
...
class TarModelExportStore(DirectoryModelExportStore):
    ...
    def __init__(self, app, out_file, gzip=True, **kwds):
        self.gzip = gzip
        self.out_file = out_file
        temp_output_dir = tempfile.mkdtemp()
        super(TarModelExportStore, self).__init__(app, temp_output_dir, **kwds)
...
class BagDirectoryModelExportStore(DirectoryModelExportStore):
    ...
    def __init__(self, app, out_directory, **kwds):
        self.out_directory = out_directory
        super(BagDirectoryModelExportStore, self).__init__(app, out_directory, **kwds)
...
class BagArchiveModelExportStore(BagDirectoryModelExportStore):
    ...
    def __init__(self, app, out_file, bag_archiver="tgz", **kwds):
        # bag_archiver in tgz, zip, tar
        self.bag_archiver = bag_archiver
        self.out_file = out_file
        temp_output_dir = tempfile.mkdtemp()
        super(BagArchiveModelExportStore, self).__init__(app, temp_output_dir, **kwds)
