...
class DescribeToolExecutionOutput:
    ...
    def __init__(self, dataset_populator: DatasetPopulator, history_id: str, hda_id: str):
    ...
    @property
    def contents(self):
        return self._dataset_populator.get_history_dataset_content(history_id=self._history_id, hda_id=self._hda_id)
...
class DescribeJob:
    ...
    def __init__(self, dataset_populator: DatasetPopulator, history_id: str, job_id: str):
        self._dataset_populator = dataset_populator
        self._history_id = history_id
        self._job_id = job_id
    ...
    def with_output(self, output: Union[str, int]):
        ...
        if by_name:
            for output in outputs:
                if output["name"] == output:
                    ...
                    return DescribeToolExecutionOutput(
                        self._dataset_populator, self._history_id, output["dataset"]["id"]
                    )
        else:
...
class DescribeToolExecution:
    ...
    def __init__(self, dataset_populator: DatasetPopulator, tool_id: str):
        self._dataset_populator = dataset_populator
    ...
    def _execute(self):
        self._execute_response = self._dataset_populator.run_tool_raw(
            self._tool_id, self._inputs, self._history_id, assert_ok=False
        )
    ...
    def assert_has_job(self, job_index: int = 0) -> DescribeJob:
        ...
        return DescribeJob(self._dataset_populator, self._history_id, job["id"])
...
class TargetHistory:
    ...
    def __init__(self, dataset_populator: DatasetPopulator, history_id: str):
        self._dataset_populator = dataset_populator
        self._history_id = history_id
...
class HasSrcDict:
    def __init__(self, src_type: str, api_object: Dict[str, Any]):
        self.src_type = src_type
        self.api_object = api_object

    @property
    def src_dict(self) -> SrcDict:
        return SrcDict({"src": self.src_type, "id": cast(str, self.api_object["id"])})
