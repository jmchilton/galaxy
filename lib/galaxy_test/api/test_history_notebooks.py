...
class TestHistoryNotebooksApi(ApiTestCase):
    ...
    def test_show_notebook(self):
        ...
        with self.dataset_populator.test_history() as history_id:
            created = self.dataset_populator.new_history_notebook(
                history_id, title="My Notebook", content="# Hello"
            )
    ...
    def test_update_notebook_content(self):
        ...
        with self.dataset_populator.test_history() as history_id:
            notebook = self.dataset_populator.new_history_notebook(
                history_id, title="Updatable", content="Version 1"
            )
    ...
    def test_update_notebook_title(self):
        ...
        with self.dataset_populator.test_history() as history_id:
            notebook = self.dataset_populator.new_history_notebook(
                history_id, title="Old Title", content="Content"
            )
            updated = self.dataset_populator.update_history_notebook(
    ...
    def test_list_revisions(self):
        ...
        with self.dataset_populator.test_history() as history_id:
            notebook = self.dataset_populator.new_history_notebook(
                history_id, title="Revisions Test", content="V1"
            )
            self.dataset_populator.update_history_notebook(
                history_id, notebook["id"], content="V2"
            )
            self.dataset_populator.update_history_notebook(
                history_id, notebook["id"], content="V3"
            )
    ...
    def test_notebook_not_found_in_other_history(self):
        ...
        with self.dataset_populator.test_history() as history1_id:
            with self.dataset_populator.test_history() as history2_id:
                notebook = self.dataset_populator.new_history_notebook(
                    history1_id, title="History 1 Notebook"
                )
                # Try to access from history2
                response = self.dataset_populator.get_history_notebook_raw(
                    history2_id, notebook["id"]
                )
                assert response.status_code == 404
