...
class ShedTestCase(ShedApiTestCase):
    ...
    def edit_repository_categories(
        ...
        populator = self._logged_in_populator or self.populator
        db_repository = self._db_repository(repository)
        current_category_ids = {str(rca.category.id) for rca in db_repository.categories}
        add_ids = {populator.get_category_with_name(name).id for name in categories_to_add}
        remove_ids = {populator.get_category_with_name(name).id for name in categories_to_remove}
    ...
    def preview_repository_in_tool_shed(
        ...
        for expected in strings_displayed or []:
            if expected == "Valid tools":
                ...
                assert (
                    expected in name or expected in metadata_str
