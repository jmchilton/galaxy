...
import { pushIgnoringNavCancel, useWindowAwareNavigation } from "./windowAwareNavigation";
...
describe("pushToFrameOrPage", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });

    it("opens a frame without blowing up when the window manager is active", () => {
        // The monkeypatch hands the URL to Galaxy.frame.add() and returns bare, so this
        // branch -- which always passes a title, and so always takes that return -- has no
        // promise coming back to it.
        mockGetGalaxyInstance.mockReturnValue({ frame: { active: true } } as never);
        mockUseRouterPush.mockReturnValue(undefined);
        const { pushToFrameOrPage } = useWindowAwareNavigation();

        expect(() => pushToFrameOrPage({ framedUrl: "/framed", inlineUrl: "/inline", title: "Report" })).not.toThrow();
        expect(mockUseRouterPush).toHaveBeenCalledWith("/framed", { title: "Report", preventWindowManager: false });
    });

    it("navigates inline when it is not", () => {
        mockGetGalaxyInstance.mockReturnValue({ frame: { active: false } } as never);
        mockUseRouterPush.mockResolvedValue(undefined);
        const { pushToFrameOrPage } = useWindowAwareNavigation();

        pushToFrameOrPage({ framedUrl: "/framed", inlineUrl: "/inline", title: "Report" });

        expect(mockUseRouterPush).toHaveBeenCalledWith("/inline");
    });
});
