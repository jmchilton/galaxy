<template>
    <b-modal modal-class="selection-dialog-modal" v-if="modalShow" visible ok-only ok-title="Close" :static="modalStatic">
        ...
        <div v-else>
            <slot name="options" v-if="optionsShow">
            </slot>
        ...
        <template v-if="!errorMessage" v-slot:modal-footer>
            <div class="w-100">
                ...
                <b-btn size="sm" class="float-right" @click="hideModal"> Cancel </b-btn>
            </div>
        </template>
    </b-modal>
</template>
