...
from base.populators import DatasetPopulator, wait_on_state, wait_on
...
class JobsApiTestCase( api.ApiTestCase ):
    ...
    def test_deleting_output_keep_running_until_all_deleted( self ):
        ...
        time.sleep( 1 )
        assert job_state().json()["state"] == "running"
    ...
    def test_purging_output_keep_running_until_all_purged( self ):
        ...
        if "file_name" in dataset_1:
            ...
            output_datset_paths_exist = os.path.exists( output_datset_paths[ 0 ] )
        else:
            output_datset_paths = []
            output_datset_paths_exist = False

        assert job_state().json()["state"] == "running", job_state().json()["state"]
        ...
        def paths_deleted():
            if not os.path.exists( output_datset_paths[ 0 ] ) and not os.path.exists( output_datset_paths[ 1 ] ):
                return True

        if output_datset_paths_exist:
    ...
    def test_purging_output_cleaned_after_ok_run( self ):
        ...
        if not output_datset_paths_exist:
        ...
        if output_datset_paths_exist:
            ...
            assert os.path.exists( output_datset_paths[ 1 ] )
            assert not os.path.exists( output_datset_paths[ 0 ] )
