...
def full_roundtrip_native(
    ...
    try:
        ...
        step_result.step_results.append(
            StepResult(
                step_id="reimport",
                tool_id=None,
                success=False,
...
def _iwc_workflows_available() -> list[str]:
    """Return list of .ga filenames in the IWC test directory."""
...
def _iwc_cache_populated(workflow_path: str) -> bool:
    """Check if all toolshed tools in a workflow are cached."""
...
def run_iwc_sweep() -> dict[str, RoundTripResult]:
    """Run per-step conversion sweep on IWC workflows using cache."""
    results: dict[str, RoundTripResult] = {}
...
def run_iwc_full_roundtrip_sweep() -> dict[str, FullRoundTripResult]:
    """Run full round-trip on IWC workflows using cache."""
    results: dict[str, FullRoundTripResult] = {}
    for filename in _iwc_workflows_available():
        path = os.path.join(IWC_WORKFLOW_DIRECTORY, filename)
        if not _iwc_cache_populated(path):
        ...
        try:
            with open(path) as f:
                workflow = json.load(f)
            step_result, diffs = full_roundtrip_native(workflow, GET_TOOL_INFO_WITH_TOOLSHED, filename)
            if not step_result.success:
                failures = [r for r in step_result.step_results if not r.success]
                error = "; ".join(f"step {f.step_id}: {f.error}" for f in failures)
                results[filename] = FullRoundTripResult(
...
class TestIWCRoundTrip:
    """Round-trip tests for IWC workflows with ToolShed tools."""

    def test_iwc_sweep_report(self):
        """Run per-step conversion sweep on IWC workflows. Reports but does not assert 100%."""
        ...
        # Report stats but don't require 100% — Phase 3.4 will drive fixes
        total = len(results)
        passed = sum(1 for r in results.values() if r.success)
        failed = total - passed
        print(f"\nIWC sweep: {passed}/{total} passed, {failed} failed")

        # Catalog unique failure classes
    ...
    def test_iwc_full_roundtrip_report(self):
        """Full round-trip on IWC workflows. Reports but does not assert 100%."""
        workflows = _iwc_workflows_available()
        if not workflows:
            pytest.skip("No IWC workflows in test/unit/workflows/iwc/")
