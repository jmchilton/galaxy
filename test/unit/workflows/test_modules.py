...
def test_mapped_parameter_passthrough_is_explicitly_unsupported():
    with pytest.raises(modules.exceptions.MessageException, match="parameter pass-through"):
        modules.SubWorkflowModule._passthrough_value_collection_type("not collection-backed")
...
def test_absent_optional_passthrough_is_not_materialized():
    ...
    replacement = modules.SubWorkflowModule._shape_passthrough_output(
        None,
        None,
        workflow_output,
        "optional_output",
...
def test_passthrough_binding_requires_complete_ordered_axis_paths(axis_indices, path_slices, expected):
    axis = modules.matching.MatchingCollectionAxis(mock_structure("list:list"), "refined")
    binding = modules.matching.MatchingCollectionBinding(
        collection=object(),
        axis_indices=axis_indices,
        axis_path_slices=path_slices,
    )

    assert modules.SubWorkflowModule._binding_covers_mapping_axes(binding, [axis]) is expected
...
def test_partial_refined_axis_passthrough_is_broadcast_over_missing_suffix():
    ...
    inner_x = modules.Tree([("P", modules.matching.leaf), ("Q", modules.matching.leaf)], list_description)
    inner_y = modules.Tree([("P", modules.matching.leaf), ("Q", modules.matching.leaf)], list_description)
    structure = modules.Tree([("X", inner_x), ("Y", inner_y)], refined_description)
    axis = modules.matching.MatchingCollectionAxis(structure, "refined")
    binding = modules.matching.MatchingCollectionBinding(
        collection=object(),
        axis_indices=(0,),
    ...
    result = modules.SubWorkflowModule._shape_passthrough_output(
        trans,
        invocation_step,
        workflow_output,
        "output",
...
def test_legacy_subworkflow_recovery_preserves_scalar_output_values():
    ...
    subworkflow_module.compute_collection_info = mock.Mock(return_value=None)
...
def test_linked_axis_identity_uses_database_invocation_id_and_round_trips():
    ...
    axis_id = modules.WorkflowModule._assign_linked_axis_identity(progress, step, collections)
    encoded = modules.matching.mapping_axis_id_to_dict(axis_id)

    assert axis_id == ("workflow-map", 83, ((107, "output"),))
    assert modules.matching.mapping_axis_id_from_dict(encoded) == axis_id
...
def test_hydrate_output_mapping_axes_excludes_output_collection_suffix():
    ...
    references = (
        modules.matching.MatchingCollectionAxisReference("outer", "list", (("outer", 0, 1),)),
        modules.matching.MatchingCollectionAxisReference("inner", "list", (("inner", 0, 1),)),
    )
    workflow_module = object.__new__(modules.WorkflowModule)
    ...
    combined = modules.matching.MatchingCollections.from_axes(axes)
...
def test_hydrate_output_mapping_axes_rejects_rectangular_split_for_ragged_structure():
    ...
    references = (
        modules.matching.MatchingCollectionAxisReference("outer", "list"),
        modules.matching.MatchingCollectionAxisReference("inner", "list"),
    )
    workflow_module = object.__new__(modules.WorkflowModule)
    workflow_module.trans = _hydration_trans()
...
def test_hydrate_output_mapping_axes_preserves_ragged_refined_axis():
    ...
    reference = modules.matching.MatchingCollectionAxisReference("refined", "list:list", components)
    workflow_module = object.__new__(modules.WorkflowModule)
    workflow_module.trans = _hydration_trans()
...
def test_hydrate_output_mapping_axes_preserves_empty_outer_axis():
    ...
    assert list(modules.matching.MatchingCollections.from_axes(axes).slice_collections()) == []


def test_hydrate_output_mapping_axes_reuses_identical_inherited_axes():
    inherited_axis = modules.matching.MatchingCollectionAxis(mock_structure("list"), "outer", (("outer", 0, 1),))
    progress = bunch.Bunch(subworkflow_collection_info=modules.matching.MatchingCollections.from_axes([inherited_axis]))
    reference = modules.matching.MatchingCollectionAxisReference.from_axis(inherited_axis)
    workflow_module = object.__new__(modules.WorkflowModule)
...
def test_top_level_recovery_does_not_recompute_collection_mapping():
    workflow_module = object.__new__(modules.WorkflowModule)
    workflow_module.compute_collection_info = mock.Mock()
    ...
    workflow_module.compute_collection_info.assert_not_called()
...
def test_subworkflow_output_mapping_preserves_explicit_empty_child_lineage():
    ...
    inherited_axis = modules.matching.MatchingCollectionAxis(mock_structure("list"), "inherited")
    collection_info = bunch.Bunch(mapping_axes=[inherited_axis])

    output_mapping = modules.SubWorkflowModule._collect_output_mapping_axes(
