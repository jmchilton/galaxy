...
def _merge_summarizes(raw_data_dicts):
    ...
    for raw_data_dict in raw_data_dicts:
        ...
        for endpoint, endpoint_metrics in these_internals.items():
            ...
            internals[endpoint]['total_time'][label].update(endpoint_metrics["total_time"])
...
def __inject_api_timing_summary_across_tests(environment):
    ...
    for test in environment['raw_data']['tests']:
        ...
        for api_endpoint, endpoint_metrics in test_endpoints.items():
            if api_endpoint not in internals:
                internals[api_endpoint] = {
                    'total_time': {'raw': []},
                }
    ...
    for api_endpoint, endpoint_metrics in api_endpoints.items():
    ...
    for _, endpoint_metrics in internals.items():
        __inject_statistics(endpoint_metrics['total_time'])
...
def __inject_statistics(from_dict):
    ...
    from_dict['median'] = statistics.median(raw_values)
    if len(raw_values) > 1:
        from_dict['stdev'] = statistics.stdev(raw_values)
    else:
        from_dict['stdev'] = "n/a"
    from_dict['mean'] = statistics.mean(raw_values)
