...
then
    ...
    if [ "$1" = "--external_tmp" ]; then
       # Following is an attempt to avoid Text file is busy errors inside
       # the running container. These occur perhaps one per API suite when
       # running on jenkins with Docker.
